...
// traits so the flow never blocks (Sat 19:00 tripwire, pre-armed).
...
export type PandaTraits = {
  ...
  interest: string; // maps to accessory
  vibe: string; // one adjective from free-text answer
  palette: string; // named palette pick
};
...
export const PALETTES: Record<string, { accent: string; soft: string }> = {
  "cosmic-purple": { accent: "#7C5CFF", soft: "#EDE8FF" },
  "nebula-teal": { accent: "#18B8A6", soft: "#DFF7F4" },
  "solar-orange": { accent: "#FF8A3D", soft: "#FFEEDF" },
  "aurora-green": { accent: "#4CC26B", soft: "#E4F7E9" },
  "plasma-pink": { accent: "#F464A8", soft: "#FDE7F1" },
  "lunar-gray": { accent: "#8A93A6", soft: "#EEF1F6" },
};
...
export function pandaPrompt(traits: PandaTraits): string {
  ...
  return (
    ...
    `${traits.vibe} expression, ${paletteOf(traits).accent} accent colors on the suit`
  );
}

function paletteOf(traits: PandaTraits) {
  return PALETTES[traits.palette] || PALETTES["cosmic-purple"];
}
...
/**
 * Flat-vector astronaut panda, deterministic from traits. Unique per member via
 * the seed (ear tilt, star field, blush, antenna). 512x512 viewBox.
 */
export function proceduralPanda(traits: PandaTraits): string {
  ...
  const accessory =
    traits.interest === "defi"
      ? `<rect x="176" y="196" width="160" height="34" rx="17" fill="${accent}" opacity="0.55"/>`
      : traits.interest === "artist"
      ? `<circle cx="200" cy="360" r="9" fill="${accent}"/><circle cx="316" cy="345" r="6" fill="${accent}" opacity="0.7"/><circle cx="240" cy="392" r="5" fill="${accent}" opacity="0.5"/>`
      : traits.interest === "researcher"
      ? `<rect x="308" y="352" width="52" height="66" rx="8" fill="${accent}"/><rect x="316" y="362" width="36" height="8" rx="3" fill="#fff" opacity="0.8"/><rect x="316" y="376" width="26" height="6" rx="3" fill="#fff" opacity="0.6"/>`
      : traits.interest === "community"
      ? `<path d="M150 350 l40 -14 v40 l-40 -14 z" fill="${accent}"/><rect x="188" y="342" width="10" height="28" rx="4" fill="${accent}"/>`
      : traits.interest === "gaming"
      ? `<rect x="212" y="382" width="88" height="40" rx="18" fill="${accent}"/><circle cx="238" cy="402" r="7" fill="#fff"/><circle cx="276" cy="402" r="7" fill="#fff"/>`
      : `<rect x="180" y="382" width="152" height="22" rx="11" fill="${accent}"/><rect x="216" y="378" width="26" height="30" rx="5" fill="${accent}"/><rect x="270" y="378" width="26" height="30" rx="5" fill="${accent}"/>`;

  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
  <rect width="512" height="512" rx="64" fill="${soft}"/>
  ${stars}
  <!-- backpack + flag pole (flag itself is overlaid by the app from the country code: ${traits.country}) -->
